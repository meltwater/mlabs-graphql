...
const createHttpLink = ({reqId, token, ...options}) => {
  const headers = {}
...
export const registerClient = (container, {
  ...
  const httpLink = ({reqId}) => createHttpLink({uri, reqId, token, ...linkOptions})
  ...
  container.register({
    ...
    [httpLinkName]: asFunction(httpLink).scoped(),
...
export const registerClients = (container, clients = {}) => {
  for (const [name, options] of Object.entries(clients)) {
    registerClient(container, {name, ...options})
  }
}
...
export default ({
  ...
  const clientLink = link || createLink({
    httpLink: createHttpLink({uri, reqId, token, ...linkOptions})
  })
