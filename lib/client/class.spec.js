...
test.beforeEach(t => {
  ...
  const apolloClient = new ApolloClient({
    link: new HttpLink({ fetch, uri: `${api}/graphql` }),
...
test.skip('query request error retry', async t => {
  const { api, client, query, data } = t.context

  nock(api)
    .post('/graphql').reply(500, { data })
    .post('/graphql').reply(200, { data })

  const res = await client(t).query(query, { retryOptions: { retries: 0 } })
  t.deepEqual(res.data, data)
})

test.skip('query request timeout retry', async t => {
  const { api, client, query, data } = t.context

  nock(api)
    .post('/graphql').reply(200, { data }).socketDelay(0)
    .post('/graphql').reply(200, { data })
