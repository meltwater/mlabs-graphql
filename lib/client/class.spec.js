...
test('unhealthy', async t => {
  ...
  await t.throwsAsync(client(t).health(), /status code 500/)
})
...
test('query request error', async t => {
  ...
  await t.throwsAsync(client(t).query({ query }), /status code 500/)
})
...
test('mutate request error', async t => {
  ...
  await t.throwsAsync(client(t).mutate({ mutation }), /status code 500/)
})

test('mutate error', async t => {
  const { client } = t.context
  await t.throwsAsync(client(t).mutate(), /Expecting a parsed GraphQL document/)
})
