...
test.beforeEach(t => {
  ...
  t.context.query = gql`query DoFoo { foo }`
  t.context.data = { foo: 'hello' }
  t.context.mutation = gql`mutation DoBar { foo }`
})
...
test('query request error retry with header', async t => {
  ...
  nock(api)
    .post(gqlPath).reply(500, { data }, { 'retry-after': 1 })
    .post(gqlPath).reply(200, { data })
...
test('query request timeout retry', async t => {
  ...
  nock(api)
    .post(gqlPath).replyWithError({ code: 'ECONNREFUSED' })
    .post(gqlPath).reply(200, { data })
