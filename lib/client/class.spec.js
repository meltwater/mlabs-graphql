...
test('query request error', async t => {
  ...
  await t.throwsAsync(client(t).query({ query }), { message: /status code 500/ })
})
...
test('mutate request error', async t => {
  ...
  await t.throwsAsync(client(t).mutate({ mutation }), { message: /status code 500/ })
})

test('mutate error', async t => {
  const { client } = t.context
  await t.throwsAsync(client(t).mutate(), { message: /Expecting a parsed GraphQL document/ })
})
