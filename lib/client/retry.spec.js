...
test.beforeEach(t => {
  t.context.createF = (numFailures = 10, code = 500) => {
    let n = 1
    return async () => {
...
test('getDelay: header in seconds', t => {
  ...
  t.is(getDelay({ statusCode, headers: { 'retry-after': 0 } }, { minTimeout: 0 }), 0)
  t.is(getDelay({ statusCode, headers: { 'retry-after': 30 } }, { minTimeout: 0 }), 30000)
  t.is(getDelay({ statusCode, headers: { 'retry-after': 30 } }, { minTimeout: 50000 }), 0)
  t.is(getDelay({ statusCode, headers: { 'retry-after': 30 } }, { minTimeout: 20000 }), 10000)
})
...
test('getDelay: header as date', t => {
  ...
  const delay = getDelay({
    statusCode,
    headers: { 'retry-after': date.toString() }
  },
  { minTimeout: 0 })
  t.log(date)
  t.true(delay > 80000)
