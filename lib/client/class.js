...
export class GraphQLClient {
  ...
  async _request (verb, req, options) {
    ...
    const { ast, reqOpts, name, logOpts, retryOptions } = normalizeArgs(
      noun,
      req,
      options,
      this.defaultOptions
    ...
    try {
      ...
      const data = await retry(() => this.apolloClient[verb](args), retryOptions)
...
const normalizeArgs = (type, req = {}, options = {}, defaultOptions = {}) => {
  ...
  const { reqOpts, logOpts, retryOptions } = getOptions(allOptions, defaultOptions)
