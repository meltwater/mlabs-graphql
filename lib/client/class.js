...
export class GraphQLClient {
  ...
  async _request (verb, req, options) {
    ...
    const log = createChildLogger(this.log, {
      ast,
      reqOpts,
      name,
      verb,
...
const normalizeArgs = (type, req = {}, options = {}, defaultOptions = {}) => {
  ...
  const ast = isSingleArg ? req[type] : req

  const allOptions = isSingleArg ? { ...req, ...options } : options
