...
import { formatApolloErrors, processFileUploads } from 'apollo-server-core'
import {
  assoc,
  complement,
  compose,
...
export default ({
  ...
  const postMiddleware = [...sharedMiddleware, koaBody(), uploads(), graphql()]
...
const graphql = () => async (ctx, next) =>
  graphqlKoa(() => ctx.state.apolloServer.createGraphQLServerOptions(ctx))(
    ctx,
    next
  )
...
const apolloServerFromState = ({ typeDefs, resolvers, schema, ...options }) => (
  ctx,
  next
) => {
  const {
