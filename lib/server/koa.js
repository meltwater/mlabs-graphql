...
export default ({
  ...
  if (graphiqlRoot) {
    router.get('graphiql', graphiqlRoot, graphiqlKoa({
      endpointURL: graphqlRoot,
      subscriptionsRoot
    }))
  }
...
const graphiqlKoa = ({ subscriptionsRoot, ...options }) => async ctx => {
  const subs = () => url.format({
    host: ctx.request.get('host'),
    protocol: ctx.request.protocol === 'https' ? 'wss' : 'ws',
    pathname: subscriptionsRoot
  })

  const opts = {
    ...(subscriptionsRoot ? { subscriptionsEndpoint: subs() } : {}),
    ...options
  }

  const query = ctx.request.query
  const body = await resolveGraphiQLString(query, opts, ctx)
...
const apolloServerFromState = (options) => (ctx, next) => {
  const { state: { container } } = ctx
  if (container) return next()
