...
export default ({
  ...
  const apolloServerMiddleware = apolloServer(
    apolloServerOptions,
    { graphqlRoot, schema, typeDefs, resolvers }
  )
  ...
  const postMiddleware = [
    koaBody(),
    detectUploads({ apolloServerMiddleware }),
    uploads(),
    ...middleware
  ]
...
const apolloServer = (options, { graphqlRoot, ...model }) => {
  let installedSubscriptionHandlers = false
  ...
  return (ctx, next) => {
    ...
    if (!installedSubscriptionHandlers) {
      const server = container.resolve('server', { allowUnregistered: true })
      if (!server) return
      apolloServer.installSubscriptionHandlers(server)
      installedSubscriptionHandlers = true
    }
...
const detectUploads = ({ apolloServerMiddleware }) => (ctx, next) => {
  if (isUpload(ctx)) return apolloServerMiddleware(ctx, next)
  return next()
}
