...
const registerModel = (
  ...
  if (remoteResolver) register('RemoteResolver', remoteResolver)
...
const createFromModel = name => (models, container) => () =>
  compose(
    flatten,
    filter(isNotNil),
...
const createResolversFromModels = name => models => ({ container }) =>
  compose(
    filter(isNotNil),
    mapObjIndexed((v, k) => resolveFromModel(container, name)(k))
  )(models)
...
const createContext = ({ container }) => ({ ctx }) => ({
  ...
  getDeps: (keys, ...args) =>
    objFromKeys(k => container.resolve(k, ...args), keys),
  ...ctx
})

const createApolloServerStart = ({ apolloServer }) => () =>
  apolloServer.willStart()
const createApolloServerStop = ({ apolloServer }) => () => apolloServer.stop()

export default (
  container,
  models = [],
  { useScopedTypeDefs = false, useScopedServer = true, ...options } = {}
) => {
  registerModels(container, models)

  const createApolloServer = ({ gqlSchema, gqlContext, gqlOptions, log }) =>
    createServer({
      ...options,
      ...gqlOptions,
      context: gqlContext,
  ...
  const typeDefsLifetime = useScopedTypeDefs
    ? Lifetime.SCOPED
    : Lifetime.SINGLETON
  const serverLifetime = useScopedServer ? Lifetime.SCOPED : Lifetime.SINGLETON
  const createResolvers = createResolversFromModels('Resolvers')
  const createRemoteResolvers = createResolversFromModels('RemoteResolvers')

  container.register({
    gqlOptions: asValue({}),
    gqlContext: asFunction(createContext)
      .inject(objOf('container'))
      .scoped(),
    gqlTypeDefs: asFunction(createTypeDefs(models, container), {
      lifetime: typeDefsLifetime
    }),
    gqlResolvers: asFunction(createResolvers(models))
      .inject(objOf('container'))
      .scoped(),
    gqlRemoteResolvers: asFunction(createRemoteResolvers(models))
      .inject(objOf('container'))
      .scoped(),
    gqlSchemaDirectives: asFunction(
      createDirectives(models, container)
    ).scoped(),
    gqlSchemas: asFunction(createSchemas(models, container)).scoped(),
    gqlSchema: asFunction(createGqlSchema(options)).scoped(),
    gqlExec: asFunction(createGqlRun(options)).scoped(),
    ...
    installApolloServerSubscriptionHandlers: asFunction(
      createInstallSubscriptionHandlers
    ).scoped()
  })
}
