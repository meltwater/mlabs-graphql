...
const createResolversFromModels = name => (models) => ({ container }) => compose(
  filter(isNotNil),
...
const createContext = ({ container }) => ({ ctx }) => ({
  ...
  getDeps: (keys, ...args) => objFromKeys(
    k => container.resolve(k, ...args),
    keys
  ),
  ...ctx
})
...
export default (
  ...
  container.register({
    ...
    gqlRemoteResolvers: asFunction(createRemoteResolvers(models)).inject(objOf('container')).scoped(),
