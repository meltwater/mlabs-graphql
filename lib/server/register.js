...
const registerModel = (container, name, model) => {
  const { typeDefs, resolvers, directive, directiveResolver, schema, query, mutation } = model
...
const createContext = ({ container }) =>
  ({ ctx }) => ({
    ...ctx,
    getDep: dep => container.resolve(dep),
    getDeps: obj => mapObjIndexed(dep => container.resolve(dep), obj)
  })
...
export default (
  ...
  const serversLifetime = useScopedServer ? Lifetime.SCOPED : Lifetime.SINGLETON

  container.register({
    gqlOptions: asValue({}),
    gqlContext: asFunction(createContext).inject(container => ({ container })).scoped(),
    ...
    apolloServer: asFunction(createApolloServer, { lifetime: serversLifetime }),
