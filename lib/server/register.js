...
const createSchema = ({
  ...
  const logger = {
    log: err => { log.error({ err }, 'GraphQL: Schema Error') }
  }
...
export default (container, models) => {
  ...
  container.register({
    typeDefs: asFunction(createTypeDefs(models, container)).singleton(),
    resolvers: asFunction(createResolvers(models)).inject(objOf('container')).scoped(),
    schema: asFunction(createSchema).scoped()
  })
}
