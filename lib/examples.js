...
const loadRequest = (type) => (base, name, vars, mapVars) =>
  Promise.all([
...
const getTransform = (transforms, type, name, vars, ...args) =>
  partialRight(pathOr(identity, [type, name], transforms), [
    name,
    vars,
    ...args
  ])
...
export const mutate = ({
  ...
  const [mutation, variables] = await loadMutation(
    dataRoot,
    name,
    vars,
    mapVars
